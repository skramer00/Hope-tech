...
export default function EquipmentDetail(){
 ...
 return <main className="wrap section equipment-detail">
  ...
  {notes.length>0&&<section className="equipment-section"><p className="eyebrow">Hope setup</p><h2>{item.slug==="studio-cameras"?"Camera assignments":"System-specific notes"}</h2><div className="equipment-assignment-grid">{notes.map(note=>{const [heading,...rest]=note.split("—");return <article key={note}><h3>{heading.trim()}</h3><p>{rest.join("—").trim()}</p></article>})}</div></section>}
